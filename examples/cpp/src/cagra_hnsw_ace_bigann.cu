...
int cagra_build_search_ace(raft::device_resources const& dev_resources)
{
  ...
  int fd = open("base.1B.u8bin", O_RDONLY);
   if (fd == -1) {
  ...
  uint32_t n_rows = 1000000; //shape[0];
  auto dataset_host_view = raft::make_host_matrix_view<const uint8_t, int64_t, raft::row_major>(dataset_ptr + header_size, n_rows, shape[1]);
